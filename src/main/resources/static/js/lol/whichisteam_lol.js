...
function startQuiz() {
    ...
    fetch('/leagueoflegends/whichisteam/startQuiz', {
        method: 'POST',
        headers: {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('/database/Team-LOL.json')  // static 경로를 통해 JSON 파일에 접근
        .then(response => response.json())
        .then(teamList => {
...
document.getElementById('player-input').addEventListener('keydown', function(event) {
    ...
    if (event.key === 'Enter') {
        ...
        fetch('/leagueoflegends/whichisteam/submitAnswer', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function showCorrect(currentPlayer, quizItem) {
...
function showWrong(currentPlayer, quizItem) {
    quizItem.style.transition = 'background-color 1s ease';
...
function endQuiz(savedQuizData) {
    fetch('/leagueoflegends/whichisteam/end', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
