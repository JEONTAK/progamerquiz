...
function startQuiz() {
    fetch('/valorant/whoareyou/startQuiz', {
        method: 'POST',
        headers: {
        ...
        .then(data => {
            ...
            playerImage.src = `/images/VALORANT/player/${data.answer.id}_${data.answer.progamerTag}.jpg`;  // 이미지 경로
...
document.getElementById('player-input').addEventListener('keydown', function (event) {
    ...
    fetch('/valorant/whoareyou/submitAnswer', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function showHint(hintResults, isCorrect, answer, guessedList, attempts) {
    ...
    const hintData = hintResults.map(hint => {
        ...
        if(hint.hintName === "nationality") {
            return {
                label,
                value: hint.inputValue,
                answerValue: hint.answerValue,
                match: hint.match,
                icon: `/images/nationality/${hint.inputValue}.png`,
                fallbackIcon: "/images/none.png",
                getArrow: null
            };
        }
...
function getHintIcon(hintName, value) {
    ...
    switch (hintName) {
        case "league":
            return `/images/VALORANT/league/${value}.png`;
        case "birth":
...
function saveQuizResult(quizData, isCorrect, attempts) {
    fetch('/valorant/whoareyou/saveResult', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
