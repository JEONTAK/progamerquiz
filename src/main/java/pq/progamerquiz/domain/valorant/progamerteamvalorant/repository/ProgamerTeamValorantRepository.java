...
public interface ProgamerTeamValorantRepository extends JpaRepository<ProgamerTeamValorant, Long> {
    ...
    @Query("SELECT new pq.progamerquiz.domain.valorant.teamvalorant.dto" +
            ".response.TeamValorantSimpleInfoResponse(pt.teamValorant.id, pt.teamValorant.name, pt.teamValorant.callName, pt.teamValorant.seasonYear) FROM ProgamerTeamValorant pt " +
    ...
    @Query("SELECT new pq.progamerquiz.domain.valorant.progamervalorant.dto" +
            ".response.ProgamerSimpleInfoResponse(pt.progamerValorant.id, pt.progamerValorant.progamerTag) FROM ProgamerTeamValorant pt " +
