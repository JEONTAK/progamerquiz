...
import pq.progamerquiz.domain.leagueoflegends.quizzes.igotyou.repository.LOLIGotYouRepository;
...
public class LOLIGotYouService {
    ...
    private final LOLIGotYouRepository lolIGotYouRepository;
    private final LOLIGotYouQuizProgamerRepository igotYouQuizProgamerRepository;

    public List<LOLIGotYouQuizResponse> setQuizLists(int totalCount) {
        LOLIGotYou loliGotYou = LOLIGotYou.create(totalCount, 0);
        LOLIGotYou savedLOLIGotYou = lolIGotYouRepository.save(loliGotYou);
        ...
        return LongStream.range(0, progamerLOLList.size())
                .mapToObj(i -> {
                    ...
                    igotYouQuizProgamerRepository.save(LOLIGotYouQuizProgamer.create(savedLOLIGotYou, progamerLOL));
                    List<TeamLOLSimpleInfoResponse> teams = progamerTeamLOLService.findTeamsByProgamerId(progamerLOL.getId());
                    return LOLIGotYouQuizResponse.of(savedLOLIGotYou.getId(), i + 1, progamerLOL.getId(), progamerLOL.getProgamerTag(), progamerLOL.getPosition(), teams);
                })
                .collect(Collectors.toList());
    }

    public LOLIGotYouResponse setQuiz(Long id, List<LOLIGotYouQuizResponse> quizList) {
        LOLIGotYou loliGotYou = lolIGotYouRepository.findById(id).orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "해당 퀴즈를 찾을 수 없습니다."));
        return LOLIGotYouResponse.of(loliGotYou.getId(), 0, loliGotYou.getTotalQuizCount(), loliGotYou.getCorrectQuizCount(), quizList);
    }
    ...
    public LOLIGotYouResultResponse saveResult(Long id, Integer correctQuizCount, Integer totalQuizCount) {
        LOLIGotYou loliGotYou = lolIGotYouRepository.findById(id).orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "해당 퀴즈를 찾을 수 없습니다."));
        lolIGotYouRepository.updateCorrectQuizCount(id, correctQuizCount);
        return LOLIGotYouResultResponse.of(loliGotYou.getId(), correctQuizCount, totalQuizCount);
    }
}
